    ...
    <div class="page-title-box">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h4 class="page-title mb-1">Cash Sale Report</h4>
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Reports</a></li>
                        <li class="breadcrumb-item active">Cash Sales Report</li>
                    </ol>
                </div>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-body">
                            {!! Form::open(array('route' => 'cashsales.report', 'method' => 'POST', 'role' => 'form', 'class' => 'needs-validation')) !!}
                            ...
                            @if(isset($cashsales))
                                <strong>Showing Cash Sale Records from {{$start}} to {{$end}}.</strong>
                                <br>
                                <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    ...
                                    @foreach($cashsales as $cashsale)
                                        @php $userInfo = \App\Models\User::where('paynumber', $cashsale->paynumber )->firstOrFail(); @endphp
                                        <tr>
                                            <td>{{$userInfo->first_name}} {{$userInfo->last_name}}</td>
                                            <td>{{$cashsale->paynumber}}</td>
                                            <td>{{$cashsale->ftype}}</td>
                                            <td>{{$cashsale->reg_num}}</td>
                                            <td>{{$cashsale->quantity}}</td>
                                            <td>{{$cashsale->created_at}}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
